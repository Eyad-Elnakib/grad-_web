...
export default function Dashboard() {
  ...
  const handleTextAnalyze = async () => {
    ...
    try {
      const startRes = await fetch("http://127.0.0.1:5001/verify-text-start", {
        method: "POST",
      ...
      if (startRes.ok) {
        ...
        const pollInterval = setInterval(async () => {
          try {
            const pollRes = await fetch(`http://127.0.0.1:5001/verify-progress/${taskId}`);
      ...
      if (res.ok) {
        ...
        if (data.ok) {
          setImageResult({
            ...
            classificationUrl: `http://127.0.0.1:5001/report?path=${encodeURIComponent(data.out_path)}`,
            segmentationUrl: data.seg_path ? `http://127.0.0.1:5001/report?path=${encodeURIComponent(data.seg_path)}` : null,
          });
        ...
        if (data.ok) {
          ...
          setPostResult({
            ...
            regions: regions.map((r: any) => ({
              ...
              classificationUrl: r.out_path ? `http://127.0.0.1:5001/report?path=${encodeURIComponent(r.out_path)}` : null,
              segmentationUrl: r.seg_path ? `http://127.0.0.1:5001/report?path=${encodeURIComponent(r.seg_path)}` : null,
            })),
  ...
  const handleVideoAnalyze = async () => {
    ...
    try {
      ...
      const res = await fetch("http://127.0.0.1:5001/classify-video", {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <div className="min-h-screen bg-black text-neutral-100 flex flex-col relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-1/2 -right-1/2 w-[1000px] h-[1000px] rounded-full bg-neutral-900/40 blur-[100px] opacity-50 mix-blend-screen" />
        <div className="absolute -bottom-1/2 -left-1/2 w-[800px] h-[800px] rounded-full bg-neutral-800/30 blur-[100px] opacity-50 mix-blend-screen" />
      </div>
      ...
      <main className="relative z-10 flex-1 container max-w-7xl mx-auto p-6 md:p-12 flex flex-col gap-8">
        <div className="flex flex-col gap-2">
          <h2 className="text-3xl md:text-4xl font-bold text-white tracking-tight">Fact-Check Anything</h2>
        ...
        <Tabs defaultValue="text" value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-4 max-w-3xl bg-neutral-900/50 border border-white/5 p-1 backdrop-blur-md rounded-xl">
            <TabsTrigger value="text" className="rounded-lg data-[state=active]:bg-neutral-800 data-[state=active]:text-white transition-all text-xs md:text-sm">
              <FileText className="w-4 h-4 mr-2 hidden md:block" />
              Text
            </TabsTrigger>
            <TabsTrigger value="image" className="rounded-lg data-[state=active]:bg-neutral-800 data-[state=active]:text-white transition-all text-xs md:text-sm">
              <ImageIcon className="w-4 h-4 mr-2 hidden md:block" />
              Image
            </TabsTrigger>
            <TabsTrigger value="post" className="rounded-lg data-[state=active]:bg-neutral-800 data-[state=active]:text-white transition-all text-xs md:text-sm">
              <Layout className="w-4 h-4 mr-2 hidden md:block" />
              Post
            </TabsTrigger>
            <TabsTrigger value="video" className="rounded-lg data-[state=active]:bg-neutral-800 data-[state=active]:text-white transition-all text-xs md:text-sm">
              <Video className="w-4 h-4 mr-2 hidden md:block" />
              Video
            </TabsTrigger>
          </TabsList>
            ...
            <div className="flex flex-col gap-6 lg:col-span-5">
              <AnimatePresence mode="wait">
                {activeTab === "text" && (
                  ...
                  >
                    <Card className="bg-neutral-900/40 border-white/10 backdrop-blur-md shadow-2xl">
                      <CardHeader>
                      ...
                      <CardFooter>
                        <Button 
                          ...
                          className="w-full bg-white text-black hover:bg-neutral-200 transition-all font-semibold rounded-lg py-6"
                        >
